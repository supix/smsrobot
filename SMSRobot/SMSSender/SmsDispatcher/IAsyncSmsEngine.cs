...
{
    public delegate void SendNotification(IAsyncSmsEngine sender, ISms sms);
    public interface IAsyncSmsEngine : IDisposable
    {
        string Name { get; set; }
        void Send(ISms sms);
        bool Busy { get; }
        event SendNotification Sent;
        event SendNotification SendError;
    }
}
